...
{
  GUI_PrintDispDec(progressRect.x0+BYTE_WIDTH*3, TEMP_Y, (int16_t)statusMsg.actHotend, 3, RIGHT);
}
static void redrawToolTag(void)
{
  GUI_PrintDispDec(progressRect.x0+BYTE_WIDTH*7, TEMP_Y, (int16_t)statusMsg.tagHotend,  3, LEFT);
}
...
void reDrawTime(void)
{
  ...
  GUI_PrintDispDec(progressRect.x0 + 3 * BYTE_WIDTH, TIME_Y, hour, 2, RIGHT);
  GUI_PrintDispDec(progressRect.x0 + 6 * BYTE_WIDTH, TIME_Y, min, 2, RIGHT);
...
//   // GUI_DispString(BED_Y, POSITION_XY_Y, (uint8_t* )"Y:");
//   // GUI_DispString(BED_X, POSITION_XY_Y, (uint8_t* )"Z:");
//    GUI_DispString(Z_OFFER, TIME_Y, (uint8_t* )"Babystep:");//
//    GUI_DispString(SPEED_X, SPEED_Y, (uint8_t* )"Sp:");
//    GUI_DispString(FLOW_X, SPEED_Y, (uint8_t* )"Fl:");
//    GUI_DispString(FAN_X, SPEED_Y, (uint8_t* )"Fan:");
//    GUI_DispString(SPEED_X + 7*BYTE_WIDTH , SPEED_Y, (uint8_t *)"%");
//    GUI_DispString(FLOW_X + 7*BYTE_WIDTH , SPEED_Y, (uint8_t *)"%");
...
void printingDrawPage(void)
{
  ...
  // printed time
  GUI_DispString(TEMP_X, TIME_Y, (uint8_t* )"T:");
  GUI_DispString(TEMP_X+BYTE_WIDTH*5, TIME_Y, (uint8_t* )":");
  //GUI_DispString(progressRect.x0+BYTE_WIDTH*7, TIME_Y, (uint8_t* )":");
  // nozzle temperature 
  GUI_DispString(TEMP_X+BYTE_WIDTH*2, TEMP_Y,(uint8_t* )":");
  GUI_DispString(TEMP_X+BYTE_WIDTH*6, TEMP_Y,(uint8_t* )"/");
  // hotbed temperature
  GUI_DispString(BED_X, TEMP_Y, (uint8_t* )"Bed:");
  GUI_DispString(BED_X+BYTE_WIDTH*7, TEMP_Y, (uint8_t* )"/");
  //Progress percentage
  GUI_DispString(BED_X, TIME_Y, (uint8_t* )"Pro:");

  // GUI_DispString(progressRect.x0, POSITION_XY_Y, (uint8_t* )"X:");
  // GUI_DispString(BED_Y, POSITION_XY_Y, (uint8_t* )"Y:");
  // GUI_DispString(BED_X, POSITION_XY_Y, (uint8_t* )"Z:");
  GUI_DispString(BABY_X, BABY_Y, (uint8_t* )"Babystep:");//
  GUI_DispString(SPEED_X, SPEED_Y, (uint8_t* )"Sp:");
  GUI_DispString(FLOW_X, FLOW_Y, (uint8_t* )"Fl:");
  GUI_DispString(BED_X, FLOW_Y, (uint8_t* )"Fan:");
  GUI_DispString(SPEED_X + 7*BYTE_WIDTH , SPEED_Y, (uint8_t *)"%");
...
// icon                       label
  ...
  {ICON_POWER_SUPPLY,         LABEL_PERCENTAGE_FLOW},
  {ICON_FAN ,                 LABEL_FAN},
